...
from switcher.switcherexceptions import SwitcherEmailSendFailure
...
class Email(object):
    ...
    def send_notification(self):
        ...
        for site_event_l in self.cloud_event_l.list():
            self._send_site_notification(site_event_l)
        self.log.debug('Leaving.')


    def _send_site_notification(self, site_event_l):

        self.log.debug('Starting for site %s.' %site_event_l.name)
        total_l = site_event_l.get_changed()
        ...
        if cc == 'None':
            cc = ''
        subject = '[Switcher3 AutoExclusion] Summary for Site {site} at Cloud {cloud} ({time})'
        subject = subject.format(site=site_event_l.name, cloud=self.cloud, time=timestamp_now())
        body = self._build_body(site_event_l)
        ...
        try:
            ...
            raise SwitcherEmailSendFailure(to, ex)
    ...
    def _build_body(self, site_event_l):
        """
            ...
            for event in event_l:
                ...
                if event.status_changed:
                    ...
                    if event.entitytype == 'Queue':
                        ...
                        if not event.queue_affected:
                            body += '        WARNING: Final value of the Queue was not affected by the change in Switcher: %s. Some other probe value dominates.\n' %event.queue_final_status.upper()
                            body += '\n'
